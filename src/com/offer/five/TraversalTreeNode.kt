...
object TraversalTreeNode {
    ...
    fun buildBinaryTreeNode(preorderTraversalNodes: IntArray, inorderTraversalNodes: IntArray): BinaryTreeNode? {
        ...
        return buildBinaryTreeNode(preorderTraversalNodes, 0, preorderTraversalNodes.size - 1, inorderTraversalNodes, 0, inorderTraversalNodes.size - 1)
    }
     ...
     */
    fun buildBinaryTreeNode(preorderTraversalNodes: IntArray, preStartPosition: Int, preEndPosition: Int,
                            inorderTraversalNodes: IntArray, inStartPosition: Int, inEndPosition: Int): BinaryTreeNode? {
        ...
        // 递归构建当前根结点的左子树，左子树的元素个数：index-inStartPosition+1个
        // 左子树对应的前序遍历的位置在[preStartPosition+1, preStartPosition+index-inStartPosition]
        // 左子树对应的中序遍历的位置在[inStartPosition, index-1]
        rootNode.left = buildBinaryTreeNode(preorderTraversalNodes,preStartPosition+1, preStartPosition+index-inStartPosition,inorderTraversalNodes,inStartPosition, index-1)
        // 递归构建当前根结点的右子树，右子树的元素个数：inEndPosition-index个
        // 右子树对应的前序遍历的位置在[preStartPosition+index-inStartPosition+1, preEndPosition]
        // 右子树对应的中序遍历的位置在[index+1, inEndPosition]
        rootNode.right = buildBinaryTreeNode(preorderTraversalNodes,preStartPosition+index-inStartPosition+1, preEndPosition,inorderTraversalNodes,index+1, inEndPosition)
        return rootNode;
    }
